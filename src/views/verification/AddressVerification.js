...
const AddressVerification = () => {
  ...
  const onSubmit = (request) => {
    if (selectedVerifications !== null) {
      if (selectedVerifications['address'].status === 'open') {
        setIsLoading(true)
        IdentityService.verifyAddress(request).then((response) => {
          setIsLoading(false)
          if (response == null) {
            ...
            if (response.success) {
              ...
              if (response.success) {
                window.location.href = '#/address/verifications'
              }
            } else {
              alert.show(response.message, {
                timeout: 20000,
        ...
        alert.show('Oops! Address Verification no longer active in your subscriptions', {
          timeout: 20000,
          type: 'error',
        })
      }
